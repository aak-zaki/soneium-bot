...
export class Helper {
  static delay = (ms, acc, msg, obj) => {
    return new Promise(async (resolve) => {
      ...
      if (acc != undefined) {
        await twist.logStatus(msg, acc, obj);
      } else {
        twist.log(msg);
      }
      ...
      const interval = setInterval(async () => {
        ...
        if (acc != undefined) {
          await twist.logStatus(
            `Waiting ${this.msToTime(remainingMilliseconds)}`,
            acc,
            obj
          );
        } else {
          twist.log(`Waiting ${this.msToTime(remainingMilliseconds)}`);
        }
